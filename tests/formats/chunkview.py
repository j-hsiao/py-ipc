...
from jhsiao.ipc.sockets.formats import chunkview

data = [b'hello world', b'goodbye\nwhatever']
vals = []
for item in data:
    L = len(item)
    pick = chunkview.bitlen_idx[L.bit_length()]
    vals.append(chunkview.encodes[pick].pack(pick, L) + item)

def test_reader_basic():
    with chunkview.Reader(io.BytesIO(vals[0])) as f:
        objs = []
    ...
    with chunkview.Reader(io.BytesIO(vals[0] + vals[1])) as f:
        objs = []
        amt = f.readinto(objs)
        assert amt == f.f.tell()
...
def test_reader_split():
    with io.BytesIO(vals[0][:1]) as buf:
        with chunkview.Reader(buf) as f:
            objs = []
            assert f.readinto1(objs) == 0
            assert f.readinto1(objs) < 0
            assert not objs
...
def test_writer():
    buf = io.BytesIO()
    with chunkview.BWriter(buf) as f:
        data = b'hello'
        f.write(data)
        assert not f
        ser = buf.getvalue()
    ...
    with chunkview.QWriter(dummy(io.BytesIO())) as f:
        data = b'hello'
        f.write(data)
        assert f
        while f:
